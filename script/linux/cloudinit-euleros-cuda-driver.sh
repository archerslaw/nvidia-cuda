...
create_nvidia_repo_euleros()
{
    ...
    echo "name=ecs-cuda-x86_6" | tee -a /etc/yum.repos.d/nvidia-centos.repo
    ...
    echo "name=ecs-driver-x86_6" | tee -a /etc/yum.repos.d/nvidia-centos.repo
    echo "baseurl=$driverurl" | tee -a /etc/yum.repos.d/nvidia-centos.repo
    echo "enabled=1" | tee -a /etc/yum.repos.d/nvidia-centos.repo
    echo "gpgcheck=0" | tee -a /etc/yum.repos.d/nvidia-centos.repo
    echo " " | tee -a /etc/yum.repos.d/nvidia-centos.repo
    echo "[ecs-package]" | tee -a /etc/yum.repos.d/nvidia-centos.repo
    echo "name=ecs-package-x86_64" | tee -a /etc/yum.repos.d/nvidia-centos.repo
...
install_nvidia_cuda_centos()
{
    ...
    if [ $cuda_file_num -eq 1 ];then
        ...
        echo "******exec \"yum list |grep cuda |grep $release |grep $cuda_big_version|grep -v update| grep -v patch | awk -F' ' '{print \$1}'\":"
        echo $cuda_file
    else
...
yum install http://mirrors.myhuaweicloud.com/epel/epel-release-latest-${version}.noarch.rpm -y >> $log 2>&1
