...
{
    begin_cuda=$(date '+%s')
    cuda_file_num=$(apt-cache search cuda | grep $release | grep $cuda_version |grep -v update | wc -l)
    if [ $cuda_file_num -eq 1 ];then
        cuda_file=$(apt-cache search cuda | grep $release | grep $cuda_version |grep -v update | awk -F' ' '{print $1}')
        echo "******exec \"apt-cache search cuda| grep $release| grep $cuda_version |grep -v update| awk -F' ' '{print \$1}'\""
        echo $cuda_file
    else
